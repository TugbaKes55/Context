      ...
      <div className="w-100">
        ...
        <div className="d-flex justify-content-end align-items-center gap-4">
          ...
          <div className="rounded overflow-hidden d-flex gap-3">
            <button className="text-white"onClick={() => addToBasket(item)}>+</button>
            <button className="text-white" onClick={() => decreaseAmount(item.id)}>-</button>
          </div>

          <h4>{item.amount} </h4>

          <button className="text-white" onClick={() => removeFromBasket(item.id)}>Sil</button>
        </div>
      </div>
    </div>
  );
